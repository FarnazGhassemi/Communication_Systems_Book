function H = vsb_lsb_filter(Fc, beta, steepness, f)
     %% Define Colors
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %% Compute the Lower Sideband (LSB) Vestigial Sideband Component
    H_beta = 0.5 * tanh(steepness * (f + Fc) / beta); 
    H_beta(abs(f) > beta) = 0;  % Zero outside |f| > beta

    %% Compute Full VSB Filter Response (for LSB) with Transitions on Both Sides
    % Apply smooth transition on both negative and positive sides
    H = 0.5 * (tanh(steepness * (f + Fc) / beta)) + 0.5 * (-tanh(steepness * (f - Fc) / beta));
    H(f < -Fc - 2*beta) = 0;  % Fully attenuate frequencies below -Fc - 2*beta
    H(f > Fc + 2*beta) = 0;    % Fully attenuate frequencies above Fc + 2*beta
    H = H / max(H);           % Normalize the response to max = 1
...
% 
% 
%      % (2) Vestigial Sideband Component 
%     subplot(2,1,2);
...
%     
%     % Save the plot
%     saveas(gcf, 'vsb_lsb_filter_response.png');  % Save as PNG
%     saveas(gcf, 'vsb_lsb_filter_response.fig');  % Save as MATLAB .fig file
end
