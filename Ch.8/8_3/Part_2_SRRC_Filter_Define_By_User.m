...
%   of Square Root Raised Cosine Filter (SRRC) with defined parameters by 
%   user in two subplots.
...
beta = input('Enter the value of beta (0 to 1): ');
...
BlueColor = [0.3010 0.7450 0.9330];
RoseColor = [0.6350 0.0780 0.1840];
...
subplot(1, 2, 1);
plot(t, combinedResponse / max(combinedResponse), 'Color', BlueColor, 'LineWidth', 1.75);
title('SRRC Pulse');
xlabel('Time (s)');
ylabel('Amplitude');
legend([' beta=', num2str(beta),  ' ,  Tsym=', num2str(Tsym),  ' ,  fs=', num2str(Fs/50)]);
...
subplot(1, 2, 2);
plot(f, abs(vals) / abs(vals(length(vals) / 2 + 1)), 'Color', RoseColor, 'LineWidth', 1.75);
