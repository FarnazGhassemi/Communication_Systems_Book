...
%    of Square Root Raised Cosine Filter (SRRC) for different Tsym & 
%    beta = 0.5 in two subplots.
...
% Define beta and Tsym values
beta = 0.5; % Example value
...
for i = 1:length(Tsym_values)
    ...
    % Plot SRRC Pulse for current Tsym
    plot(t, combinedResponse / max(combinedResponse), 'LineWidth', 1.75, 'Color', colors(i, :), 'DisplayName', ['T_{sym} = ', num2str(Tsym)]);
end
...
% Calculate the bandwidth of the SRRC Filter for the last Tsym value
BW = 0.5*(1+beta)*(Fs/50);
