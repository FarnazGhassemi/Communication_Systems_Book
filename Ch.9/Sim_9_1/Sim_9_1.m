...
figure;
%sgtitle('Signal, Flipped+Shifted, and Convolution for Various Signals')

for i = 1:size(signals, 1)
    p_func = signals{i, 1};
    name = signals{i, 2};
    
    % Original signal p(t) over [0, T]
    p_vals_short = p_func(t);
    
    % Zero-pad to match length of t_full
    p_vals = [p_vals_short, zeros(1, length(t_full) - length(p_vals_short))];

    % Flipped and shifted version h(T - t)
    h_vals_short = p_func(T - t);
    h_vals = [h_vals_short, zeros(1, length(t_full) - length(h_vals_short))];

    % Convolution (output length = 2*N - 1), pad to match t_full if needed
    conv_result = conv(p_vals_short, h_vals_short) * dt;
    conv_result = [conv_result, zeros(1, length(t_full) - length(conv_result))];  % Pad if needed

    % Plotting
    row = (i - 1) * 3;

    subplot(5,3,row+1);
    plot(t_full, p_vals, 'b', 'LineWidth', 2.25);
    xlim([0 2*T]); title([name ' - Original p(t)']);
    xlabel('t'); ylabel('Amplitude'); grid on;

    subplot(5,3,row+2);
    plot(t_full, h_vals, 'b', 'LineWidth', 2.25);
    xlim([0 2*T]); title(['Matched Filter']);
    xlabel('t'); ylabel('Amplitude'); grid on;
